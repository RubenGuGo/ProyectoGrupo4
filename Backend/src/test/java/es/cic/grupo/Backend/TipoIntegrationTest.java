...
public class TipoIntegrationTest {
    ...
    public void setup() {
        ...
        tipo = new Tipo(15l, "Tipo de prueba", "Descripción de prueba");
    }
    ...
    @Test
    public void testGetTipoByNombre() throws Exception {
        tipoRepositorio.save(tipo);

        MvcResult result = mockMvc.perform(get("/api/tipos/nombre/" + tipo.getNombre()))
                .andExpect(status().isOk())
                .andReturn();

        Tipo fetchedTipo = objectMapper.readValue(result.getResponse().getContentAsString(), Tipo.class);
        assertThat(fetchedTipo.getNombre()).isEqualTo(tipo.getNombre());
    }
